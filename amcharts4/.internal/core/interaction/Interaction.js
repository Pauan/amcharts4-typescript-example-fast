...
var Interaction = /** @class */ (function (_super) {
    ...
    Interaction.prototype.addGlobalEvents = function () {
        ...
        if (!this._globalEventsAdded) {
            ...
            this._disposers.push(addEventListener(document, this._pointerEvents.pointercancel, function (ev) { _this.handleGlobalPointerUp(ev); }));
 ...
 */
    ...
    Interaction.prototype.handleOver = function (io, pointer, ev) {
        ...
        // touch.
        // (this is disabled for now because we need to log all hover pointers
        // in order to fogure out if all are out when up event happens)
                    ...
                    if (!ignoreBehavior && pointer.touch && !this.moved(pointer, this.getHitOption(io, "hitTolerance"))) {
                        ...
                        if (behavior == "leave") {
                            ...
                            this._delayedEvents.out.push({
                                ...
                                pointer: pointer,
                                event: ev,
                                keepUntil: new Date().getTime() + 500
                            });
                            return;
                        }
    ...
    Interaction.prototype.handleDown = function (io, pointer, ev) {
        ...
        // Also prevent default action so that users can still use touch gestures
        // on non-movable items
        if (io.focusable !== false && this.getHitOption(io, "noFocus")) {
            if (this.focusedObject) {
                $dom.blur();
            }
            if ((io.draggable || io.swipeable || io.trackable) && !this.isGlobalElement(io)) {
                ev.preventDefault();
            }
        }
        io.isDown = true;
    ...
    Interaction.prototype.handleUp = function (pointer, ev) {
        ...
        if (pointer.touch) {
            $iter.each(this.overObjects.backwards().iterator(), function (io) {
                // Is this pointer relevant to element?
                if (io && io.overPointers.contains(pointer)) {
